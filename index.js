...
app.post('/upload', upload.single('file'), function(req, res) {
  if(req.file){
    ...
    const header = "<h1>File uploaded as "+req.file.fieldname+"</h1>";
    const subheader = `<h2><a href=${url}>${url}</a></h2>`;
    const qrlibrary = "<script src='https://cnd.rawgi.com/davidshimjs/qrcodejs/gh-pages/qrcode.min.js' ></script>"
...
app.post('/api/upload', upload.single('file'), function(req, res) {
  if(req.file){
    const filename = req.file.filename;
    const uri = `http://${req.headers.host}/file/${filename}`
    return res.status(200).send(url);
  }
  else
    res.status(400).send("Mo file specified!");
});
